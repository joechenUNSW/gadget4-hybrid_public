...
def main():
    ...
    if (hasattr(args,'input_delta_nu') and args.input_delta_nu != None):
        ...
        delta_nu_dat = np.loadtxt(args.input_delta_nu,delimiter=',',usecols=range(0,Nstreams+1))
        k_delta = delta_nu_dat[:,0]
        delta = delta_nu_dat[:,streamno]**2
        data_delta = np.array([delta,delta])
    ...
    if (hasattr(args,'input_theta_nu') and args.input_theta_nu != None):
        if hasattr(args,'input_delta_nu'):
            ...
            theta_nu_dat = np.loadtxt(args.input_theta_nu,delimiter=',',usecols=range(0,Nstreams+1))
            growthrate = theta_nu_dat[:,streamno]/delta_nu_dat[:,streamno]
            data_growthrate = np.array([delta_nu_dat[:,0],growthrate])
